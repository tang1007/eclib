...
{
	template<class _CLS>
	class AioIpcClient : public cThread // Asynchronous auto reconnect IPC client, for compatible with windows XP, use the select model
	{
	public:
	...
	public:
		...
		bool ipc_post(ec::vector<uint8_t> *pd, int timeovermsec = 100) // post send data
		{
...
#ifdef _WIN32
				SOCKET s = netio_tcpconnect("127.0.0.191", _port, 4, true);
#else
				SOCKET s = afunix_connect( _port, 4, true);
#endif
				if (INVALID_SOCKET == s)
					return;
	...
	*/
	...
	public:
		...
		bool ipc_post(uint32_t ucid, void* pdata, size_t bytesize, int timeovermsec = 100) // post data, warning: zero copy, direct put pdata pointer to send buffer
		{
		...
		bool ipc_post(uint32_t ucid, vector<uint8_t> *pvd, int timeovermsec = 100) // post data, warning: zero copy, direct put pdata pointer to send buffer
		{
	...
	private:
		SOCKET listen_port(unsigned short wport, const char* sip = nullptr)
		{
			...
			{
				if (_plog)
					_plog->add(CLOG_DEFAULT_ERR, "IPC port %u socket error!", wport);
				fprintf(stderr, "IPC port %u socket error!\n", wport);
				return false;
			}
#ifdef _WIN32
			netaddr.sin_family = AF_INET;
			netaddr.sin_addr.s_addr = inet_addr("127.0.0.191");
			...
			if (bind(sl, (const sockaddr *)&netaddr, sizeof(netaddr)) == SOCKET_ERROR)
			{
				::closesocket(sl);
				if (_plog)
					_plog->add(CLOG_DEFAULT_ERR, "IPC port [%d] bind failed with error %d", wport, errno);
				fprintf(stderr, "ERR:IPC port [%d] bind failed with error %d\n", wport, errno);
			...
			if (listen(sl, SOMAXCONN) == SOCKET_ERROR)
			{
				::closesocket(sl);
				if (_plog)
					_plog->add(CLOG_DEFAULT_ERR, "IPC port %d  listen failed with error %d", wport, errno);
				fprintf(stderr, "ERR: IPC port %d  listen failed with error %d\n", wport, errno);
				return INVALID_SOCKET;
			}
			return sl;
		}
