...
#pragma once //2018/9/19 新版,使用 c11_xpoll.h
...
public:
    ...
    inline static  int NetInt(int v)
    {
        return (int)NetUInt(v);
    }
    ...
    inline static  short NetShort(short v)
    {
        return (short)NetUShort(v);
    }
};
    ...
    */
    int Do_shmsg(unsigned int ucid, const void* pmsg, unsigned int msglen, unsigned int seqno, tArray<unsigned char>* pout)
    {
        ...
        if (!strcmp(sod, "connect")) { //连接后，客户端发来connect命令,服务端应答onconnect命令，发送随机信息要求客户端计算摘要
            ...
            if (nr != 0) { //错误
                sret = "onconnect,-1,error or disable!";
                MakePkg(sret, strlen(sret), rpcmsg_sh, rpccomp_none, seqno, NULL, pout);
                return -1;
            }
            char sinfo[44];
...
protected:
    virtual void  OnConnect(unsigned int ucid, const char* sip)
    {
        if (_plog)
            _plog->add(CLOG_DEFAULT_MSG, "Server(%d) UCID %u IP=%s TCP connected!", _wport, ucid, sip);
        _pcli->Add(ucid, sip);
    };
    ...
    virtual bool	OnReadBytes(unsigned int ucid, const void* pdata, unsigned int usize) //返回false表示要断开客户端连接.
    {
        ...
        while (nr == 1) {
            ...
            if (_msgs.GetSize() > 0)
                SendToUcid(ucid, _msgs.GetBuf(), _msgs.GetSize());
    ...
    */
    int PutMsg(unsigned int ucid, const void* pdata, size_t usize)
    {
        ...
        int nret = SendToUcid(ucid, _putmsg.GetBuf(), _putmsg.GetSize());
...
{
public:
    cRpcBaseServer(memory* pmem, uint32_t maxconnum, cLog* plog) : cTcpServer(pmem, maxconnum, plog)
    {
        _plog = NULL;
...
public:
    ...
    */
    int PutMsg(unsigned int ucid, const void* pdata, size_t usize)
    {
        ...
        int nret = SendToUcid(ucid, putmsg.GetBuf(), putmsg.GetSize());
        return nret;
    }
protected:
